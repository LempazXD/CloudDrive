...
Архитектурно: **модульный монолит**: композиционный корень — `src/Bootstrapper/CloudDrive.Api`, бизнес-функциональность выносится в независимые модули (`src/Modules/`), а общие примитивы — в `src/Shared/`. Реализованы модули **Auth** (регистрация с подтверждением email по коду, вход, JWT + refresh-токены, выход из одной сессии или всех сразу, восстановление и смена пароля) и **Files** (папки и файлы — создание, переименование, перемещение, удаление; загрузка и скачивание файлов через presigned URL в S3-совместимое объектное хранилище).
